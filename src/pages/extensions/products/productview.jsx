          ...
          {displayPage &&
            <main id="productViewInnerMain">
              ...
              <div id="productViewMainDiv">
                ...
                {productDetails &&
                  <section>
                    {productDetails.map(({ id, cartColor }) => {
                      if (cartColor) {
                        return (
                          <div key={id} id="productColorView" className="productDivs">
                            <p className="productColorText">Color {cartColor}</p>
                            <main className="productColorMain">
                              <div className="productColorMainDiv" style={{ backgroundColor: "#00A849" }}></div>
                              <p className="productColorMainText">{cartColor}</p>
                            </main>
                          </div>
                        )
                      }
                    })}
                    {productDetails.map(({ id, cartSize }) => {
                      if (cartSize) {
                        return (
                          <div key={id} id="productSizeView" className="productDivs">
                            <div className="productSizeDiv">
                              <p className="productSizeText">Size {cartSize}</p>
                            </div>
                            <main className="productSizeMain">
                              <p className="productSizeMainText">{cartSize}</p>
                            </main>
                          </div>
                        );
                      }
